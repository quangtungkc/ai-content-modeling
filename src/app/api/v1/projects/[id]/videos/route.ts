...
import { enqueueProjectVideosWithVeoApi } from "@/modules/generation/google-api-pipeline";
...
export async function GET(request: Request, context: Context) {
  ...
  try {
    ...
    if (searchParams.get("status") === "1") {
      ...
      return Response.json({ data: { status: queueJob.status === "succeeded" ? "completed" : queueJob.status, provider: "veo-api", queueJobId, videos, error: queueJob.error }, requestId });
    }
...
export async function POST(request: Request, context: Context) {
  ...
  try {
    ...
    return Response.json({ data: await enqueueProjectVideosWithVeoApi(id, session.userId, body.channelId, slots), requestId }, { status: 202 });
  } catch (error) {
    return toErrorResponse(normalize(error), requestId);
  }
}
