...
export async function POST(request: Request, context: Context) {
  ...
  try {
    const session = await getRequiredSession();
    const { id } = await context.params;
    const body = await request.json() as { provider?: "flow" | "gemini"; channelId?: string; slots?: Array<{ sceneNumber?: number; label?: string; visualBlock?: string; actionBlock?: string; audioBlock?: string; englishPrompt?: string; aspectRatio?: string; targetDuration?: number | null }> };
