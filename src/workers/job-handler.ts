...
export async function handleJob(name: string, payload: Record<string, unknown>) {
  ...
  if (name === "video.project.generate") {
    if (payload.provider === "veo-api") return generateProjectVideosWithVeoApi(String(payload.projectId), String(payload.userId), String(payload.channelId), payload.slots as Parameters<typeof generateProjectVideosWithVeoApi>[3]);
    return generateProjectVideosWithFlowBrowser(String(payload.projectId), String(payload.userId), String(payload.channelId), payload.slots as Parameters<typeof generateProjectVideosWithFlowBrowser>[3], undefined, `desktop-flow-legacy-videos:${String(payload.batchId ?? payload.projectId)}`);
  }
