...
export class OpenAIProvider implements AIProvider {
  ...
  generateIdeas(input: AIInput & { analysis: VideoAnalysis; artStyle?: string }) { return this.request(`Generate exactly ONE original modeling idea in Vietnamese. Preserve only the source mechanism, never surface details, characters, setting, wording, or sequence. If Channel DNA says a fixed main-character reference image is available, make that recurring protagonist the same central character in every scene and do not invent a replacement protagonist; the reference image will be attached during visual generation. Return one modelingDirections item with title, coreConcept, script, characterDesign, setting, artStyle, sourceMechanism, whatIsPreserved, whatIsChanged, targetMarketAdaptation, similarityRisk, whyWorthDeveloping, and postText. postText must be a short ready-to-publish caption written in channelDNA.language, culturally suitable for channelDNA.targetCountry, based on the new video situation, and must not contain hashtags. Selected art style: ${input.artStyle ?? "Use the channel's existing art style"}.`, input, modelingIdeasSchema); }
  developIdea(input: AIInput & { analysis: VideoAnalysis; idea: ModelingDirection; aspectRatio?: string }) { return this.request(`Develop this modeling idea into a production-ready package in Vietnamese. If a fixed main-character reference image is available in Channel DNA, use the same recurring protagonist in every scene and explicitly preserve its identity; the reference image will be attached to Flow when generating visuals. Keep character identity and background continuity across every scene. Selected frame size: ${input.aspectRatio ?? "9:16"}. Return only JSON matching the development schema with deconstruction, artDirection, characterDesign, backgroundDesign, storyboard, and safetyReview.`, input, developedIdeaSchema); }
