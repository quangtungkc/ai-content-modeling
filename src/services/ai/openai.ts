...
export class OpenAIProvider implements AIProvider {
  ...
  generateIdeas(input: AIInput & { analysis: VideoAnalysis }) { return this.request("Generate exactly 3 to 5 original modeling directions. Preserve only the source mechanism, never surface details, characters, setting, wording, or sequence. Return only JSON with sourceMechanism, whatIsPreserved, whatIsChanged, targetMarketAdaptation, similarityRisk, and whyWorthDeveloping.", input, modelingIdeasSchema); }
