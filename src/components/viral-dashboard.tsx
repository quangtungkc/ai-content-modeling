...
export function ViralDashboard() {
  ...
  function buildFlowImageSlots(project = contentProject): ImageSlot[] {
    ...
    const referenceInstruction = selectedChannel?.mainCharacterImageUrl
      ? `Use the attached fixed main-character reference image only to preserve the exact identity, silhouette, face, colors, clothing, and proportions of the channel's recurring protagonist. Never reproduce the reference-sheet layout, labels, poses, panels, or text. Compose the requested scene as one natural full-frame image.`
      : "No fixed main-character reference is available; keep the approved project character design consistent.";
    return [...project.scenes].sort((left, right) => left.sceneNumber - right.sceneNumber).map((scene) => ({
      kind: "scene" as const,
      sceneNumber: scene.sceneNumber,
      label: `Cảnh ${scene.sceneNumber}`,
      aspectRatio: "9:16",
      prompt: toFlowSafePrompt(`${referenceInstruction}\nCreate only scene ${scene.sceneNumber} as one single full-frame vertical image. Do not create a collage, storyboard, character sheet, contact sheet, split panel, multiple variations, captions, labels, or written text.\n${scene.englishPrompt}\nVisual: ${scene.visualBlock}\nAction: ${scene.actionBlock}\nKeep the approved fixed main character, environment, lighting, and art direction consistent with the project. Make the visual treatment entirely original. Aspect ratio: 9:16. Generate exactly one final image.`),
    }));
  }
  ...
  async function generateAllProjectVideos(project = contentProject, images = generatedImages): Promise<Record<string, string> | null> {
    if (!project) return null;
    const sceneSlots: VideoSlot[] = project.scenes.map((scene) => ({ ...scene, label: `Video cảnh ${scene.sceneNumber}`, aspectRatio }));
    if (sceneSlots.some((scene) => !images[`scene-${scene.sceneNumber}`])) {
      setContentProjectError("Hãy tạo đầy đủ ảnh phân cảnh trước khi tạo video.");
      return null;
    }
          ...
          <section className="max-h-[90vh] w-full max-w-2xl overflow-y-auto rounded-2xl bg-white p-6 shadow-2xl">
            ...
            <div className="mt-6 rounded-xl border border-[#d8e3f1] bg-[#f7f9fc] p-4">
              ...
              {contentProject && <div className="mt-4 space-y-3">
                ...
                {flowImageSlots.length > 0 && <div className="rounded-xl border border-[#d8e3f1] bg-[#f7f9fc] p-4"><p className="font-extrabold text-[#0b3262]">Quy trình Google Flow</p><p className="mt-1 text-sm leading-6 text-[#6883aa]">Mỗi phân cảnh dùng một project Flow riêng với Image · 9:16 · Nano Banana Pro · x1. App chỉ chuyển sang cảnh tiếp theo sau khi ảnh hiện tại đã tải và lưu thành công.</p>{flowImageMessage && <p className="mt-2 text-sm font-semibold text-[#0b5799]">{flowImageMessage}</p>}<p className="mt-2 text-xs text-[#6883aa]">Nếu lần đầu dùng, hãy đăng nhập Google Flow trong cửa sổ mở ra. Không xử lý CAPTCHA tự động.</p></div>}
