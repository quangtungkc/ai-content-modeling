...
export async function addCompetitor(channelId: string, userId: string, input: unknown) {
  await assertChannelOwner(channelId, userId);
  const { url, source, displayName } = competitorInputSchema.parse(input);
  ...
  if (existing) {
    if (source === "FOLLOWING_PAGE" && existing.status === "ACTIVE") {
      return db.competitor.update({
        where: { id: existing.id },
        data: { discoverySource: source, ...(displayName && existing.displayName === existing.handle ? { displayName } : {}) },
      });
    }
    if (existing.status === "INACTIVE") {
      return db.competitor.update({
        where: { id: existing.id },
        data: { ...normalized, displayName: displayName || normalized.handle, discoverySource: source, status: "ACTIVE" },
      });
    }
    throw new AppError("DUPLICATE_COMPETITOR", "Competitor URL này đã tồn tại trong Channel.", 409);
  }
  return db.competitor.create({ data: { channelId, ...normalized, normalizedUrl: normalized.url, displayName: displayName || normalized.handle, avatar: null, discoverySource: source } });
}
