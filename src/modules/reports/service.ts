...
export async function prepareDailyReport(channelId: string, now = new Date(), ai?: AIService) {
  ...
  for (const item of viral) {
    try {
      const analysis = await aiService.analyzeVideo({ channelDNA: { name: channel.name, topic: channel.topic, subTopic: channel.subTopic, targetCountry: channel.targetCountry, language: channel.language, audience: channel.audience, contentStyle: channel.contentStyle, visualStyle: channel.visualStyle, videoDuration: channel.videoDurationSec, hasDialogue: channel.hasDialogue, creativeInstructions: channel.creativeInstructions, mainCharacterImageAvailable: Boolean(channel.mainCharacterImageKey), mainCharacterImageName: channel.mainCharacterImageName, timezone: channel.timezone }, video: { id: item.video.id, url: item.video.url, caption: item.video.caption, thumbnailUrl: item.video.thumbnailUrl, publishedAt: item.video.publishedAt?.toISOString(), duration: item.video.duration } });
